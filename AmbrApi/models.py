            ...
            for desc in promote.description:
                ...
                for param in re.findall(r'{param\d+:(?:F1P|F1|P|I|F2P|F2)}', param_str):
                    ...
                    param_str = param_str.replace(param,
                                                  f'{str(round(p * 100, 1))}%' if fm in ['F1P', 'P', 'F2P',
                                                                                         'F2'] else str(
                                                      int(p)))
    ...
    def get_material_list(self) -> list[MaterialItemCount]:
        ...
        for item, num in material_ids.items():
            if mat := load_json(RAW / 'materials.json')['items'].get(item):
                material_list.append(MaterialItemCount(material=MaterialItem.parse_obj(mat), count=num))
...
class Weapon(BaseModel):
    ...
    def attack(self) -> int:
        ...
        return int(
            round(
                self.upgrade.prop[0].initValue * load_json(RAW / 'weapon_curve.json')['90']['curveInfos'][
                    self.upgrade.prop[0].type] +
    ...
    def extra_prop_value(self) -> float:
        ...
        if self.upgrade.prop[1].propType:
            return round(
                self.upgrade.prop[1].initValue * load_json(RAW / 'weapon_curve.json')['90']['curveInfos'][
                    self.upgrade.prop[1].type], 3)
        else:
            return 0
    ...
    def get_material_group(self, material_list: Optional[list[MaterialItemCount]] = None):
        """素材分组"""
    ...
    def save_detail(self) -> dict:
        return {
            ...
            '基础攻击力': self.attack,
            '副属性':     {
                '名称': self.extra_prop_name,
                '数值': self.extra_prop_value
            },
            '技能':       self.skill,
            'buff':       {
                '常驻':   [],
...
class MonsterEntry(BaseModel):
    ...
    def get_attack_num(self, level: str) -> int:
        return int(self.prop[1].initValue * load_json(RAW / 'monster_curve.json')[level]['curveInfos'][
            self.prop[1].type])
